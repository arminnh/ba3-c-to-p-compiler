...
{  //cout << "begin " << stack->fStore[stack->fMP + fQ]->getValue() << endl; 
    ...
    if((stack->fStore[stack->fMP + fQ]->getValue() + stack->fStore[stack->fMP + fQ + 2]->getValue()) < 0)	
    {
		...
		for(int i = 1; i <= stack->fStore[stack->fMP + fQ + 1]->getValue(); ++i)
		{
			...
			if((-effectiveaddress > 0) || ((-effectiveaddress -1) < stack->fNP))
