	...
	{	
		...
		for(int i = 0; i <= fQ - 1; ++i)
		{
			if(typeid(p1) == typeid(*stack->fHeap[-i - newaddress - 1]))
				stack->fStore.push_back(new StackInteger(*(dynamic_cast<StackInteger*>(stack->fHeap[-i - newaddress-1]))));
			if(typeid(p2) == typeid(*stack->fHeap[-i - newaddress - 1]))
				stack->fStore.push_back(new StackBoolean(*(dynamic_cast<StackBoolean*>(stack->fHeap[-i - newaddress-1]))));
			if(typeid(p3) == typeid(*stack->fHeap[-i - newaddress - 1]))
				stack->fStore.push_back(new StackAddress(*(dynamic_cast<StackAddress*>(stack->fHeap[-i - newaddress-1]))));			
			if(typeid(p4) == typeid(*stack->fHeap[-i - newaddress - 1]))
				stack->fStore.push_back(new StackCharacter(*(dynamic_cast<StackCharacter*>(stack->fHeap[-i - newaddress-1]))));
			if(typeid(p5) == typeid(*stack->fHeap[-i - newaddress - 1]))
				stack->fStore.push_back(new StackReal(*(dynamic_cast<StackReal*>(stack->fHeap[-i - newaddress-1]))));
			if(typeid(p6) == typeid(*stack->fHeap[-i - newaddress - 1]))
				stack->fStore.push_back(new StackElement(*(dynamic_cast<StackElement*>(stack->fHeap[-i - newaddress-1]))));
		}
	}
	else
	{
