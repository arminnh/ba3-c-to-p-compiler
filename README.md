A compiler which can compile a subset of C to p-machine bytecode. _Project for the [Compilers](https://www.uantwerpen.be/popup/opleidingsonderdeel.aspx?catalognr=1001WETTEL&taal=en&aj=2015) course at the University of Antwerp._
...
*   **python3 c2p.py [-h] [-save-ast] [-save-symbol-table] [-t] [-q] [-o O] filename**  
    Takes a C file _filename_ and attempts to compile it  
...
*   **build.sh** or **build.bat**  
    Generates a lexer and a parser used to build the AST for the C code
*   **test.sh** or **test.bat**  
...
*   **visualize.sh filename**  
    Shows the concrete parse tree generated for _filename_
...
The compiler does not stop when it encounters an error, instead it marks the node in the AST which generated an error and skips this node in future passes. If there were errors and/or warnings, they will be displayed at this point. Go to the 'Errors and Warnings' section to get an idea of which messages can be given.
...
A couple of interactive test programs are provided under src/tests/programs/
